...
class ContentTest extends TestCase
{
    ...
    {
        return $this->postJson('/api/v1/sessions', ['city' => 'budapest', 'game_size' => 'small', 'config' => ['rounds' => 1]])->json('id');
    }
    ...
    public function test_registered_user_authors_and_lists_custom_curses(): void
    {
        ...
        $this->postJson('/api/v1/my/curses', ['name' => 'The Snail', 'cost' => 'Walk only', 'description' => 'No transit for 10 min', 'duration_minutes' => 10, 'requires_proof' => true])
            ->assertCreated()->assertJsonPath('name', 'The Snail')->assertJsonPath('duration_minutes', 10)->assertJsonPath('requires_proof', true);

        $this->getJson('/api/v1/my/content')->assertOk()->assertJsonPath('curses.0.name', 'The Snail');
    }
    ...
    public function test_guests_cannot_author(): void
    {
        ...
        $this->postJson('/api/v1/my/curses', ['name' => 'X'])->assertStatus(403);
    }

    public function test_a_user_cannot_edit_or_delete_another_users_content(): void
    {
        Sanctum::actingAs($author = User::factory()->create());
        $id = $this->postJson('/api/v1/my/curses', ['name' => 'Mine'])->json('id');

        Sanctum::actingAs(User::factory()->create());
        $this->patchJson("/api/v1/my/curses/{$id}", ['name' => 'Hijacked'])->assertStatus(403);
        $this->deleteJson("/api/v1/my/curses/{$id}")->assertStatus(403);
    }

    public function test_delete_own_curse(): void
    {
        Sanctum::actingAs(User::factory()->create());
        $id = $this->postJson('/api/v1/my/curses', ['name' => 'Temp'])->json('id');

        $this->deleteJson("/api/v1/my/curses/{$id}")->assertOk();
    ...
    public function test_host_custom_curse_is_scoped_to_their_deck(): void
    {
        ...
        $curseId = $this->postJson('/api/v1/my/curses', ['name' => 'Host Curse'])->json('id');
    ...
    {
        Sanctum::actingAs($host = User::factory()->create());
        $questionId = $this->postJson('/api/v1/my/questions', ['title' => 'Red door', 'prompt' => 'Send a photo of a red door.'])->json('id');
        $ownSession = $this->createSession();

        $ownCatalog = collect($this->getJson("/api/v1/sessions/{$ownSession}/questions")->json())->pluck('id');
        $this->assertTrue($ownCatalog->contains($questionId));

        // A different host's game must not include it.
        Sanctum::actingAs(User::factory()->create());
        $otherSession = $this->createSession();
        $otherCatalog = collect($this->getJson("/api/v1/sessions/{$otherSession}/questions")->json())->pluck('id');
